...
本说明用于指导如何安全地升级依赖并执行例行安全审计，确保 Outlook Manager 在长期维护中保持可预测、可验证的状态。
...
5. **记录变更**
   ...
   - 若遇到兼容性问题，记录在 `docs/DEPENDENCY_MAINTENANCE.md` 供后续参考
...
项目根目录提供 `scripts/security_scan.sh`，按顺序执行：
...
| 频率 | 任务 |
|------|------|
| 每周 | 运行 `scripts/security_scan.sh`，同步三方漏洞公告 |
