...
│   │   ├── services/     # 业务逻辑层
│   │   ├── db/           # 数据库操作（Mixin 架构）
│   │   ├── auth/         # 认证与安全（JWT、加密、OAuth）
│   │   ├── core/         # 核心功能（异常、限流、消息）
│   │   ├── utils/        # 工具函数
│   │   └── migrations/   # 数据库迁移
│   ├── configs/          # system_config.json（初始化用）
│   └── requirements.txt  # Python 依赖
├── frontend/             # React + TypeScript + Vite 前端
├── docker/               # Dockerfile、compose、部署脚本
...
完整 API 文档见 [API 参考](./api-reference.md)，重点更新：
...
- `scripts/maintenance/encrypt_existing_accounts.py`：对已有账户执行加密迁移。
- `scripts/maintenance/view_login_audit.py`：查看登录审计日志。
- `scripts/maintenance/cleanup_email_cache.py`：清理过期邮件缓存。
- `scripts/test_rate_limiting.py`、[安全机制](./security.md)：登陆安全验证。
...
4. 更新 [API 参考](./api-reference.md)、`README` 后再提交。
