...
Outlook Manager 实现了完善的登录安全机制,包括:
...
在 `backend/app/rate_limiter.py` 中可调整:
...
```bash
python3 scripts/view_login_audit.py
```
...
如需为特定 IP 设置白名单,可在 `rate_limiter.py` 中添加:
...
- `backend/app/rate_limiter.py`: 频率限制和审计核心逻辑
- `backend/app/routers/auth.py`: 登录端点集成
- `scripts/view_login_audit.py`: 审计日志查看工具
