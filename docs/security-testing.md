...
```bash
# 检查 settings.py
grep "ALLOWED_ORIGINS" backend/app/settings.py

# 预期输出: ALLOWED_ORIGINS 来自环境变量（例如默认包含 http://localhost:5173）
...
from app.auth.security import encrypt_value, decrypt_value, is_encrypted
...
```bash
# 1. 检查迁移脚本
ls -l scripts/maintenance/encrypt_existing_accounts.py
...
### 代码检查
- ✅ `backend/app/auth/jwt.py` 强制检查 JWT_SECRET_KEY
- ✅ `backend/app/settings.py` 包含 ENABLE_LEGACY_ADMIN_TOKEN 和 ALLOWED_ORIGINS
- ✅ `mail_api.py` 使用 ALLOWED_ORIGINS 而非通配符
- ✅ `routers/accounts.py` 不记录敏感请求数据
- ✅ 服务层与 `backend/app/auth/security.py` 模块协同集成了加密/解密逻辑
